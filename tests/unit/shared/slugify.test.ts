...
describe("slugify()", () => {
  describe("NF-UNIT-401: PascalCase / camelCase → lowercase", () => {
    it("lowercases AcmeCorp → acmecorp", () => {
      expect(slugify("AcmeCorp")).toBe("acmecorp")
    })

    it("lowercases mixed-case AcmePro → acmepro", () => {
      expect(slugify("AcmePro")).toBe("acmepro")
    })
  ...
  describe("NF-UNIT-402: spaces become hyphens", () => {
    it("single space → single hyphen", () => {
      expect(slugify("Acme Pro")).toBe("acme-pro")
    })
...
describe("uniqueSlug()", () => {
  describe("NF-UNIT-407: no collision — returns base slug unchanged", () => {
    it("returns base when existing set is empty", () => {
      expect(uniqueSlug("acmepro", new Set())).toBe("acmepro")
    })

    it("returns base when existing set contains different slugs", () => {
      expect(uniqueSlug("acmepro", new Set(["acmecorp", "myapp"]))).toBe("acmepro")
    })
  })

  describe("NF-UNIT-408: first collision → appends -2", () => {
    it("appends -2 when base is taken", () => {
      expect(uniqueSlug("acmepro", new Set(["acmepro"]))).toBe("acmepro-2")
    })
  })
  ...
  describe("NF-UNIT-409: multiple collisions → increments suffix", () => {
    it("appends -3 when -2 is also taken", () => {
      expect(
        uniqueSlug("acmepro", new Set(["acmepro", "acmepro-2"]))
      ).toBe("acmepro-3")
    })

    it("appends -4 when -2 and -3 are taken", () => {
      expect(
        uniqueSlug("acmepro", new Set(["acmepro", "acmepro-2", "acmepro-3"]))
      ).toBe("acmepro-4")
    })
  })
  ...
  describe("NF-UNIT-410: sparse gaps are NOT reused — strictly increments", () => {
    it("uses -4 even when -3 is free if -2 is taken", () => {
      ...
      const existing = new Set(["acmepro", "acmepro-2"])
      // -3 is free, so it returns -3
      expect(uniqueSlug("acmepro", existing)).toBe("acmepro-3")
    })

    it("skips all occupied suffixes in order", () => {
      const existing = new Set(["acmepro", "acmepro-2", "acmepro-3", "acmepro-4"])
      expect(uniqueSlug("acmepro", existing)).toBe("acmepro-5")
    })
  })
})
