...
| FEAT-014 | Notification Preferences — per-event email vs console-only control | M | P2 | Not Started | `feat/FEAT-014-notification-preferences` | [spec](specs/FEAT-014-notification-preferences.md) |
...
| QE-07 | Auto-resolve smoke canary cases at webhook ingestion | XS | P1 | ✅ Done (2026-04-09) | `isSmokeCanary()` helper in `external-ingress.ts` detects `senderName="smoke-test"` or `channelContext.source="smoke-test"`. Bypasses triage dispatch; transitions `enriching→triaged→resolved` inline; returns `canary:true` in `ExternalIngestResult`. Route handler forwards `canary:true` in HTTP response. `smoke-pipeline.ts` exits 0 immediately on `canary:true`. 6 unit tests (NF-UNIT-EXT-08–13) + 3 integration tests (NF-INT-EXT-10–12). |
