...
| FEAT-017-D | Extend webhook handlers: (a) `subscription.updated` for `saas_subscription` → update plan + trigger license reissue. (b) `subscription.deleted` for trial cancels: no 30-day grace, set `deprovision_after=trial_end`. (c) `customer.updated` → sync email to `provisionings.customer_email`. | S | P1 | Not Started | Covers upgrade, downgrade, trial cancel behavior, email sync. See spec §3.3 and §7 C1+C3. |
...
| FEAT-017-G | Short reactivation window: add `reactivation_deadline` column to `provisionings` (7 days from cancellation). If customer re-subscribes before deadline, reset status to `active`. Account page shows countdown. After deadline, normal deprovisioning proceeds. | S | P2 | Not Started | See spec §7 C2. Requires new migration + account page UI change + re-subscribe flow. |
...
| ID | Title | Size | Priority | Status | Notes |
|----|-------|------|----------|--------|-------|
| FEAT-018 | Open-Core Fleet Separation (umbrella) | M | P2 | Not Started | [spec](specs/FEAT-018-open-core-fleet-separation.md) |
| FEAT-018-A | Generate Ed25519 keypair; embed public key in `src/fleet/operator-key.ts`; create `scripts/generate-operator-keypair.ts`; add `NESTFLEET_OPERATOR_KEY` to `docker-compose.prod.yml` env block | XS | P2 | Not Started | One-time key generation — private key stored securely by owner, never committed. Public key hardcoded in binary. |
| FEAT-018-B | Create `src/fleet/` directory; move 7 `src/provisioning/` files to `src/fleet/provisioning/`; add `LICENSE-FLEET.md` headers; update internal imports within moved files | S | P2 | Not Started | Pure file move — no functional change. Prerequisite for all other sub-tasks. |
| FEAT-018-C | Move `src/api/v1/owner.ts` → `src/fleet/api/owner.ts`; move `src/api/v1/saas.ts` → `src/fleet/api/saas.ts`; update `src/api/index.ts` import paths | XS | P2 | Not Started | Mount points (`/api/v1/owner`, `/api/v1/saas`) unchanged. |
| FEAT-018-D | Extract fleet Stripe webhook paths from `src/billing/webhook.ts` → `src/fleet/billing/webhook-fleet.ts`; community webhook delegates to fleet handler for `saas_signup` / `saas_subscription` event types | XS | P2 | Not Started | Community billing path (`workspace_billing`) stays in `src/billing/webhook.ts`. Clean split with no cross-contamination. |
| FEAT-018-E | Move 4 fleet workers to `src/fleet/workers/` (`provisioning-worker`, `deprovision-scheduler`, `fleet-health-worker`, `license-reissue-worker`); update `src/index.ts` worker registration | XS | P2 | Not Started | Workers wrapped in `isFleetOperatorAuthorized()` gate — only start if key verified. |
| FEAT-018-F | Startup gate in `src/index.ts`: call `verifyOperatorKey(config.NESTFLEET_OPERATOR_KEY)` when `PROVISIONING_ENABLED=true` (skip in `NODE_ENV=test`); only mount fleet routes and start fleet workers if key valid; log clear error if key missing or invalid | S | P2 | Not Started | Key check is async at startup. Invalid key → fleet disabled, community routes unaffected, process continues. |
| FEAT-018-G | Create `LICENSE-FLEET.md` at repo root (placeholder — "all rights reserved, contact for commercial license"); add `// SPDX-License-Identifier: LicenseRef-NestFleet-Fleet-1.0` headers to all `src/fleet/**/*.ts`; update `README.md` with open-core section | XS | P2 | Not Started | Legal placeholder sufficient before public flip. Formalize with exact license terms when legal entity is registered. |
| FEAT-018-H | Run full test suite (unit + integration + type check) after all moves; fix any broken imports | XS | P2 | Not Started | Must pass: `npm test`, `npm run test:integration`, `npm run lint`. No functional regressions allowed. |
