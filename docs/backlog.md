...
| OPS-M-03 | ollama-ai-provider CVE — @ai-sdk/provider-utils uncontrolled resource consumption | XS | P2 | ⏳ Monitoring | Low severity. No fix available upstream. `ollama-ai-provider` pins an old `@ai-sdk/provider-utils ≤3.0.97`. CI audit gate excludes dev deps (`--omit=dev`) — this is a runtime dep so will surface when fixed upstream. **Resolution path tracked as OPS-M-08** — migrate the ollama/self-hosted branch off `ollama-ai-provider` onto the OpenAI-compat provider, which drops this dependency entirely and also unblocks the AI SDK v7 Dependabot quartet (#170/#175/#176/#164). See [spec](specs/OPS-M-08-ollama-openai-compat-migration.md). |
...
| OPS-M-06 | Stripe SDK v21 → v22 (Dependabot PR #47, superseded by #112) | S | P3 (deferred — paid features on hold) | ⏸️ Deferred / re-confirmed (2026-07-08) | **Do not action until paid/billing comes off hold.** Full prep below so the fix needs no re-investigation. See detailed analysis in **OPS-M-06 notes** under the backlog table. Re-confirmed 2026-07-08 against PR #112 (`21.0.1 → 22.3.0`, pinned version now `2026-06-24.dahlia`): same 4 sites, same root cause, analysis still holds — closed #112 with a pointer to this entry rather than re-deriving. |
| OPS-M-07 | Console CI never ran `eslint .` — 78-issue lint backlog cleared, CI gated | M | P1 | ✅ Done (2026-07-08) | Console CI job ran `tsc --noEmit` + `npm audit` only, never `npm run lint` — 78 errors/warnings had built up on `main` undetected. Cleared all 78 (32 unescaped-entity, 16 `<a>`→`<Link>`, 30 React Compiler `react-hooks/*` rules) and added a `Lint` step to `.github/workflows/ci.yml`'s console job. 3 were real bugs, not just lint noise: `NodeDetailPanel.tsx` called `useCallback` after an early return (Rules of Hooks violation); `compliance/page.tsx`'s `nextId` counter mutated a discarded `useState` value instead of using its setter (silently reset every render); `owner/new-customer/page.tsx` defined a component inside render (remount/focus-loss every parent render). 5 sites called `Date.now()` during render for trial/expiry countdowns — extracted to a shared `useNow()` hook (hourly refresh) rather than freezing at mount, which would have made countdowns stop decrementing on long-lived pinned tabs. Verified via lint + type-check + `next build` + live Chrome check (landing page, docs nav, auth redirect) before push; full CI (incl. new lint step) and VPS deploy green post-push. See [[reference_console_ci_lint_gap]] and [[feedback_major_dep_bump_verify]]. |
| OPS-M-08 | Migrate ollama/self-hosted provider off `ollama-ai-provider` to OpenAI-compat — unblocks AI SDK v7 quartet (Dependabot #170/#175/#176/#164) | L | P2 | ⏳ TODO | `ollama-ai-provider@1.2.0` hardcodes spec-v1 (`this.specificationVersion = "v1"`); `ai@7`'s model resolver only accepts `"v3"`/`"v4"` and throws `UnsupportedModelVersionError` for anything else — confirmed against real `node_modules` output on a throwaway branch with the quartet installed. `tsc --noEmit` + full unit suite are a **false green** today (pre-existing `as unknown as LanguageModel` cast + full `vi.mock()` of `ollama-ai-provider` in the test file mask the incompatibility). Deferred quartet PRs re-open once this lands. Also closes **OPS-M-03** (CVE monitoring entry) by dropping the dependency entirely. See **OPS-M-08 notes** below and [spec](specs/OPS-M-08-ollama-openai-compat-migration.md). |
...
#### OPS-M-08 notes — ollama-ai-provider → OpenAI-compat migration prep (analysis 2026-08-04)

> Context captured from a deep analysis so the migration can be executed without re-investigation. Blocks 4 held Dependabot PRs: `ai` 6→7 (**#170**), `@ai-sdk/openai` 3→4 (**#175**), `@ai-sdk/google` 3→4 (**#176**), `@ai-sdk/anthropic` 3→4 (**#164**).

**Root cause (confirmed on a throwaway branch with the quartet installed):**
`src/agents/llm-provider.ts`'s `buildModel()` routes the `"ollama"`/`"self-hosted"` case through `createOllama()` from `ollama-ai-provider@1.2.0`, which hardcodes `this.specificationVersion = "v1"` (`node_modules/ollama-ai-provider/dist/index.js:372,712`). `ai@7`'s model resolver only accepts `"v3"`/`"v4"` and throws `UnsupportedModelVersionError` for anything else (`node_modules/ai/dist/index.js:1082-1085`). Merging the quartet as-is would pass CI clean and then hard-crash in production the instant any product configured with `provider: "ollama"` or `"self-hosted"` runs an agent job (triage, auto-reply, outage-routing, etc. — see `ACTION_TIERS` in `llm-provider.ts`).

**Why CI doesn't catch it today (false green):**
- `tsc --noEmit` is silenced by a pre-existing `as unknown as LanguageModel` cast at `llm-provider.ts:172`, which used to surface `TS2322: Type 'LanguageModelV4' is not assignable to type 'LanguageModel'` before the cast was added.
- `tests/unit/agents/llm-provider.test.ts` fully `vi.mock()`s `ollama-ai-provider`, so the unit suite never exercises the real return shape.
- Both checks need to change as part of this migration, not just the runtime code, or the false-green condition just moves to the new provider.

**Pre-validated fix path (spike already done — `docs/reference/ollama-structured-output-compat.md`, validated 2026-06-30 against live Ollama 0.30.10 + qwen2.5-coder:7b with `ai@7.0.2` + `@ai-sdk/openai@4.0.4`):**
Drop `ollama-ai-provider` entirely. Route ollama/self-hosted through `createOpenAI({ baseURL: "<host>/v1", apiKey: apiKey ?? "ollama" })` — `@ai-sdk/openai@4` already speaks spec-v3/v4. Plain `generateText` and strict `Output.object` structured output (SDK default `json_schema` mode) both work through Ollama's `/v1` endpoint.

**Residual gap — confirmed Ollama backend limitation, not an SDK/shim bug:**
Ollama's llama.cpp grammar enforces JSON shape/types/enums but not numeric `min`/`max`, regex, or zod `.refine()`. Measured 5/6 valid over 6 runs with a `min`/`max` constraint present. Affected schemas: `confidenceScore: z.number().min(0).max(1)` in `src/agents/impl/outage-routing.ts`, `change-prep.ts`, `known-issue-match.ts`; `z.array(z.number().int().min(1).max(4))` in `auto-reply.ts`. The maintained native fork (`ollama-ai-provider-v2`) hits the identical gap — waiting for it buys nothing on reliability, only a marginally cleaner integration, and it hard-requires `zod@^4.0.16` as a peer (would force an unwanted zod v3→v4 migration across every schema in the codebase). Not worth waiting for.

**Migration scope:**
1. `src/agents/llm-provider.ts` — replace `createOllama` with `createOpenAI({ baseURL, apiKey: apiKey ?? "ollama" })` in the `"self-hosted"`/`"ollama"` branch of `buildModel()`; drop the `as unknown as LanguageModel` cast (should type-check cleanly once the provider returns a real spec-v3/v4 model).
2. `package.json` — remove `ollama-ai-provider` dependency; this closes **OPS-M-03**.
3. Merge the quartet (#170, #175, #176, #164) once the migration lands — they're only held on this.
4. Decide + implement structured-output hardening for the 4 affected schemas — retry-only (existing `AI_NoObjectGeneratedError` retry in `run-agent.ts` already masks most single-run failures) vs. schema-relaxation-plus-clamping (drop unenforceable `.min/.max`, clamp/normalize in code, e.g. `confidence = Math.min(1, Math.max(0, raw > 1 ? raw / 100 : raw))`) vs. both.
5. `tests/unit/agents/llm-provider.test.ts` — mock `@ai-sdk/openai`'s `createOpenAI` for the ollama/self-hosted branch instead of `ollama-ai-provider`; check whether the existing openai/azure-openai mock (`mockCreateOpenAIFn`) can be reused/shared and just assert `baseURL` for the ollama case.
6. **Mandatory live verification before merge** (standing rule: never trust green CI alone for a provider-spec change) — redo the compat-doc's validation, this time through `getLlmProviderForProduct` / the real code path, not a standalone script.
7. Cross-version check: `LLM_BASE_URL` / `LLM_PROVIDER` / `LLM_API_KEY` env vars and the DB-driven `product.llm_config.provider === "ollama"|"self-hosted"` path must keep working identically from the console/API caller's perspective — this is an internal provider-construction swap, not a config surface change. No new env vars expected, no compose/Caddyfile impact expected — verify explicitly, don't assume.

**Testing plan (full detail in the spec's "Testing Plan" section) — two concrete behavior-change risks found by reading the actual SDK code, not just the type diff:**
- `createOpenAI` falls back to `process.env.OPENAI_API_KEY` when `apiKey` is left `undefined` (`node_modules/@ai-sdk/openai/dist/index.js:6883-6887`) — the ollama/self-hosted branch must always pass a non-empty placeholder, or a real OpenAI key sitting in env (e.g. because another product on the instance uses OpenAI) gets silently picked up. Needs a unit test with `OPENAI_API_KEY` explicitly set in the test env, proving it's not read.
- `createOpenAI`'s default `baseURL` is `https://api.openai.com/v1` — a live external endpoint — vs. `ollama-ai-provider`'s safe local default `http://127.0.0.1:11434/api` (`node_modules/ollama-ai-provider/dist/index.js:768`). Without an explicit local default on the new branch, a misconfigured self-hosted product silently talks to real OpenAI instead of failing to connect. Needs a unit test proving the branch defaults `baseURL` locally when unset.

Also: `run-agent.ts`'s existing `AI_NoObjectGeneratedError` retry (`tests/unit/agents/run-agent.test.ts` NF-UNIT-360…363) only models **truncation** (doubles output budget) — it has never been tested against the Ollama grammar-constraint failure mode (complete, valid-shape response with an out-of-range number), because `ollama-ai-provider` was mocked out. Whichever hardening approach is chosen (spec item 4) needs its own test, not reliance on the truncation tests. Recommend also adding a standing regression test asserting `llm-provider.ts` contains no `as unknown as` casts — that's the exact escape hatch that hid the v1-vs-v3/v4 mismatch from `tsc` for the whole quartet.
