/**                                                                                                                                                                                                                                                                                               
  * GitHub webhook route — POST /webhooks/github/events/:productId                                                                                                                                                                                                                                 
  *                                                                                                                                                                                                                                                                                                
  ...
  */                                                                                                                                                                                                                                                                                               
  ...
  githubWebhookRouter.post("/events/:productId", async (c) => {                                                                                                                                                                                                                                     
    ...
    try {                                                                                                                                                                                                                                                                                           
      ...
      if (product) {
        const raw = (product.ci_config ?? {}) as Record<string, unknown>                                                                                                                                                                                                                            
        ciConfig = {
        ...
        if (typeof raw["github_webhook_secret"] === "string") {
          ciConfig.github_webhook_secret = raw["github_webhook_secret"]
          webhookSecret = decryptSecret(raw["github_webhook_secret"] as string) ?? webhookSecret
        }
    ...
    // Reject unsigned requests — webhook secret is required                                                                                                                                                                                                                                        
    if (!webhookSecret) {
      logger.warn({ productId, eventType }, "GitHub webhook: no webhook secret configured — rejecting")                                                                                                                                                                                             
    ...
    // Only dispatch if the event type is one we care about
    if (eventType === "pull_request" || ciConfig?.track_deployments) {
      return dispatchEvent(c, productId, eventType, payload, ciConfig)
    }

    logger.info({ productId, eventType }, "GitHub webhook: event type not tracked, skipping dispatch")
    return c.json({ message: "Event type not tracked" }, 200)
  })
