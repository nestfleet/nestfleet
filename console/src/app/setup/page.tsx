  ...
  const ctaItems = [
    {
      ...
      description: "Upload docs, FAQs, and known issues so the AI has context for every case.",
    ...
    {
      label: "Set Up a Channel",
      description: "Connect email, Slack, or another support channel to start receiving cases.",
    ...
    {
      label: "Go to Queue",
      description: "Explore the console and see how cases will flow through your product.",
      path: `/p/${slug}/queue`,
      primary: false,
      icon: (
        ...
        {ctaItems.map((item) => (
          <button
            ...
            className={`w-full flex items-start gap-3.5 rounded-xl border p-4 text-left transition-all ${
              item.primary
                ? "border-indigo-200 bg-indigo-50 hover:bg-indigo-100 ring-1 ring-indigo-200"
                : "border-gray-200 bg-white hover:bg-gray-50"
            }`}
          >
            <span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${
              item.primary ? "bg-indigo-600 text-white" : "bg-gray-100 text-gray-500"
            }`}>
            ...
            <div className="min-w-0">
              <p className={`text-sm font-semibold ${item.primary ? "text-indigo-900" : "text-gray-900"}`}>
                {item.label}
              </p>
...
export default function SetupPage() {
  ...
  async function handleFinish() {
    ...
    try {
      ...
      // Set nf_last_product cookie immediately so subsequent navigation within
      // the console resolves the product without a stale JWT.
      ...
      // Always show the Done screen — let the user pick their next step.
      setStep(5);
    } catch (err) {
